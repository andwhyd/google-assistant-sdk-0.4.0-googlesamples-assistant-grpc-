...
def register_model(ctx, model, type, trait,
                   manufacturer, product_name, description, verbose):
    ...
    if verbose:
        ...
        click.echo('model successfully registered')
...
@click.option('--verbose')
@click.pass_context
def register_device(ctx, device, model, nickname, verbose):
    session = ctx.obj['SESSION']
    ...
    if verbose:
        ...
        click.echo('device instance successfully registered')
